...
class fun(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if randomGen1 == 1:
            ...
            elif "gn" == message.content.lower():
                await message.channel.send(f"GN {username}", reference=message)

            elif "jinx" == message.content.lower() or "cat" == message.content.lower():
                await message.channel.send(
                    "https://cdn.discordapp.com/attachments/754245502538743808/957467294156615680/9k.png"
                )

            elif "dorchadas" == message.content.lower():
                await message.channel.send(
                    "https://media.discordapp.net/attachments/952203557724094525/957235469131874314/ezgif.com-gif-maker_2.gif"
                )

            elif "kiby" == message.content.lower():
                await message.channel.send(
                    "https://tenor.com/view/kirby-kirby-cooked-kirby-massage-kirby-pat-massage-gif-25195626"
                )

            elif (
                "fish" == message.content.lower()
                or "fishing" == message.content.lower()
            ):
                await message.channel.send(
                    "https://cdn.discordapp.com/attachments/754245502538743808/958202794211414096/FO-32LgVUAMXEO-.png"
                )

            elif "fire" == message.content.lower():
                react = "🔥"
                await message.add_reaction(react)

            elif "upvote" == message.content.lower():
                react = "<:upvote:957698832513237022>"
                await message.add_reaction(react)

            elif "downvote" == message.content.lower():
                print(f"{funmode}")
        ...
        for x in message.mentions:
            ...
            if x == self.client.user:
                ...
                if randomGen == 1:
                    await message.channel.send(
                        f"whatttttttttttttttt", reference=message
                    )
                elif randomGen == 2:
                    await message.channel.send(
                        f"<@!{message.author.id}>", reference=message
                    )
                elif randomGen == 3:
                    await message.channel.send(
                        f"stop pinging me!!!!!", reference=message
                    )
            ...
            if message.channel.id in channels:
                ...
                if (not message.content) or message.attachments:
                    ...
                    if match:
                        ...
                        await avamember.avatar.save(
                            f"deltarune/temp/{message.author.id}.png"
                        )
                        ...
                        d1.text(
                            (150, 30), "*  " + input, fill=(255, 255, 255), font=thefont
                        )
                        ...
                        await message.channel.send(
                            file=discord.File(
                                f"deltarune/temp/{message.author.id}final.png"
                            )
                        )
        ...
        if ctx.author.id == ID:
            async with ctx.typing():
                if arg == "on":
                    ...
                    await ctx.send(
                        f"Set the slowmode delay in this channel to {3} seconds!"
                    )
                elif arg == "no":
                    funmode = False
                    await ctx.reply(f"3:")
